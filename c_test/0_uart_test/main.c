...
{
    while (/* TODO 2 */(*uart_stat_reg) & 0x8);               // 如果 TX FIFO 已满，则等待已有字符发送完毕
    *uart_tx_fifo = (unsigned int)c;    // 写入新的字符na到TX FIFO
}

char uart_getc(void)
{
    while (/* TODO 3 */!((*uart_stat_reg) & 0x1));               // 如果 RX FIFO 为空, 则等待接收上位机的字符
...
int main()
{
    ...
    print_str("2024311426 Test #0 - UART simple test:\n\r");
