...
{
    while (/* TODO 3 */(*uart_stat_reg) & 0x8);               // 如果 TX FIFO 已满，则等待已有字符发送完毕
...
static char uart_getc(void)
{
    if (rx_buf_ptr >= rx_buf_cnt)
    {
        ...
        while (/* TODO 4 */!((*uart_stat_reg) & 0x1));    // 如果 RX FIFO 为空, 则等待接收上位机的字符

        // 只要 RX FIFO 非空且 rx_buf 未满，则不断读取 RX FIFO 中的字符到 rx_buf 中
        while (!(/* TODO 5 */(*uart_stat_reg) & 0x1) && rx_buf_cnt < RX_FIFO_SIZE)
